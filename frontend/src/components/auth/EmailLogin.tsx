...
interface EmailLoginProps {
  onLogin?: (email: string) => void; // optional, now handled inside
}
...
export const EmailLogin = ({ onLogin }: EmailLoginProps) => {
  ...
  // 🔹 Call API to save email in MongoDB
  const saveEmailToDB = async (email: string) => {
    try {
      setLoading(true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const success = await saveEmailToDB(email.trim().toLowerCase());
    if (!success) return;

    // Call parent onLogin if passed
    onLogin && onLogin(email.trim().toLowerCase());
  };
